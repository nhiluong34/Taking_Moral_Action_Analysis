...
## Collapse Chapter 1 and take out References
```{r}
chapter1 <- str_c(p1c1, collapse = " ")
chapter1 <- str_remove(chapter1, "References(.*)")
```
...
## Subsections (still working on)
```{r}
extract_section <- function(text, chap, start) {
  pattern <- str_c(chap, "\\.", start, "(?!\\.)", "(.*)(?=", chap, "\\.", start+1, "(?![^()]*\\)))")
  paragraph <- str_extract(text, pattern)
}

extract_section_end <- function(text, chap, start) {
  pattern <- str_c(chap, "\\.", start, "(?!\\.)", "(.*)")
  paragraph <- str_extract(text, pattern)
}

section <- vector("list", 7)
for (i in 1:7) {
  if (i < 7) {
    section[i] <- test_function(chapter1, 1, i) 
  } else if (i == 7) {
      section[i] <- test_function_end(chapter1, 1, i)
    }
}
...
three_parts <- rbind(part1_tbl, part2_tbl, part3_tbl)
...
```{r}
write_csv(three_parts, "three_parts.csv")
```
